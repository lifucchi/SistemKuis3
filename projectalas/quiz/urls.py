...
urlpatterns = [
    ...
    path('quiz/<int:pk>/', views.TopicList.take_quiz, name='take_quiz'),
    # path('quiz/<int:pk>/', views.TakeQuiz.as_view(), name='take_quiz'),

    re_path(r'^quiz/(?P<quiz>[0-9]+)/(?P<quiz_taker>[0-9]+)/(?P<question_id>[0-9]+)/$', views.TopicList.question, name='question'),
