...
class mfuzzy:
    ...
    def fuzzy(self):
        x_discriminant, x_difficulty2 , x_prob , x_response, ability = self.membership()

        #infrence rule
        ability_ctrl = self.rule(x_discriminant, x_difficulty2 , x_prob , x_response, ability)

        theability = ctrl.ControlSystemSimulation(ability_ctrl)

        #fuzzification
        theability.input['discriminant'] = float(self.a)
        theability.input['difficulty2'] = float(self.b)
        theability.input['probabilitas'] = float(self.p)
        theability.input['response'] = self.r
    ...
    def rule(self, x_discriminant, x_difficulty2, x_prob, x_response, ability):
